...
function print_category($category = '') {
	...
	if($cfg_reg['kt_min'] != $cfg_reg['kt_max']) {
		echo '<ul>※<b>'.$cfg_reg['kt_min'].'</b>～<b>'.$cfg_reg['kt_max'].'</b>個まで選択できます<br>';
	} else {
		echo '<ul>※<b>'.$cfg_reg['kt_max'].'</b>個選択してください<br>';
	}
	echo '※各カテゴリの詳細は「<a href="sitemap.php" target="_blank">カテゴリ一覧</a>」を参考にしてください<br>'."\n";

	for($category_no = 1; $category_no <= $cfg_reg['kt_max']; $category_no++) {
		$select = ' selected';
		echo '<select name="Fkt'.$category_no.'" size="7">';
		if(isset($category_list[$category_no])) {
			echo '<option value="'. $category_list[$category_no] . '"'.$select.'>' . full_category($category_list[$category_no]) . "</option>\n";
			$select='';
		}
		...
		foreach($rowset as $row) {
			if($_POST['changer'] == 'admin' || !$row['regist']) {
				echo '<option value="'.$row['path'].'">' . full_category($row['path']) . "</option>\n";
			}
		}
		echo '</select><br><br>'."\n";
	}
