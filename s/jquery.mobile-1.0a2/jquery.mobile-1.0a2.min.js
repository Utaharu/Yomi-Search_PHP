...
c,c,true);k.preventDefault()}}});a("a").live("click",function(k){var p=a(this),r=p.attr("href").replace(location.protocol+"//"+location.host,""),y=p.is("[target]"),A=y||/^(:?\w+:)/.test(r)||p.is("[rel=external]");y=p.is("[target]");if(r==="#")return false;z=p.closest(".ui-btn").addClass(a.mobile.activeBtnClass);if(A||!a.mobile.ajaxLinksEnabled){n(true);if(y)e.open(r);else location.href=r}else{y=p.data("transition");A=p.data("戻る");var J=!p.is("[data-rel="+a.mobile.nonHistorySelectors+"]");H=p.attr("data-rel");
